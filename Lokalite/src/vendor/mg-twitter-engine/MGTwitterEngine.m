...
{
    if (![_connections objectForKey:requestIdentifier])
        return;
...
#import "NSString+SBJSON.h"  // for JSONValue
...
- (void)connectionDidFinishLoading:(MGTwitterHTTPURLConnection *)connection
{
    ...
    if (![connection error] && [self _isValidDelegateForSelector:@selector(requestSucceeded:)])
		[_delegate requestSucceeded:[connection identifier]];
    else if ([connection error] && [self _isValidDelegateForSelector:@selector(requestFailed:withError:)]) {
        NSError *error = [connection error];
        if ([connection data]) {
            // errors contain an error string, so we want to extract that
            NSString *s = [[NSString alloc] initWithData:[connection data]
                                                encoding:NSUTF8StringEncoding];
            NSDictionary *d = [s JSONValue];
            NSMutableDictionary *userInfo =
                [[[connection error] userInfo] mutableCopy];
            if (!userInfo)
                userInfo = [[NSMutableDictionary alloc] init];
            NSString *msg = [d objectForKey:@"error"];
            if (!msg)
                // when twitter is overloaded there is no error key
                msg = NSLocalizedString(@"twitter.error.overloaded", nil);
