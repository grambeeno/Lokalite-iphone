...
//
// jad: Adding delegate so that it can be set to nil is -dealloc methods of
// client classes. In some case the JSON parser that parses asynchronously,
// which will call a delegate method on this class. The parser retains an
// instance of this class to insure it isn't destroyed while in use. This class
// will in turn call its delegate method, but that object has been destroyed.
// Exposing this accessor allows the client to set the delegate to nil in its
// -dealloc moethod, ensuring no methods will be called on the destroyed object.
//
...
// Getting retweets
- (NSString *)getRetweetsSinceID:(NSString *)updateID page:(int)page count:(int)count;
...
+ (NSInteger)twitterOverloadedErrorCode;
