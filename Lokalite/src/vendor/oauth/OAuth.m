...
- (void) synchronousRequestTwitterTokenWithCallbackUrl:(NSString *)callbackUrl {
    ...
    BOOL requestFailed = !![request error];
	if (requestFailed) {
		if ([self.delegate respondsToSelector:@selector(requestTwitterTokenDidFail:error:)]) {
			[delegate requestTwitterTokenDidFail:self error:[request error]];
		}
	} else {
		...
		for (NSString *component in responseBodyComponents) {
            ...
            if ([subComponents count] == 2)
                ...
                // jad: Parameters weren't in the response, so just throw up a
                // Twitter error. I'm fixing this bug because I first saw it
                // when getting over capacity messages, so that's the message
                // I'm putting here, whether or not it's actually true.
                NSString *key = NSLocalizedDescriptionKey;
                NSString *msg =
                    NSLocalizedString(@"twitter.error.overloaded", nil);
                NSDictionary *userInfo =
                    [NSDictionary dictionaryWithObject:msg forKey:key];
                NSError *error = [NSError errorWithDomain:@"Twitter"
                                                     code:500
                                                 userInfo:userInfo];
                
                if ([self.delegate respondsToSelector:@selector(requestTwitterTokenDidFail:error:)])
                    [self.delegate requestTwitterTokenDidFail:self error:error];
        ...
        if (!requestFailed && [self.delegate respondsToSelector:@selector(requestTwitterTokenDidSucceed:)]) {
            [self.delegate requestTwitterTokenDidSucceed:self];
        }
	} 
}
...
- (void) synchronousAuthorizeTwitterTokenWithVerifier:(NSString *)oauth_verifier {
	...
	if (requestFailed) {
		...
		if ([self.delegate respondsToSelector:@selector(authorizeTwitterTokenDidFail:error:)]) {
			[delegate authorizeTwitterTokenDidFail:self error:[request error]];
		}
	} else {
		NSArray *responseBodyComponents = [[request responseString] componentsSeparatedByString:@"&"];
		for (NSString *component in responseBodyComponents) {
            ...
            if ([subComponents count] == 2)
                ...
                if ([self.delegate respondsToSelector:@selector(authorizeTwitterTokenDidFail:error:)]) {
                    [delegate authorizeTwitterTokenDidFail:self error:error];
                }
